...
def calculate_drum_layers_radiuses(
    outer_drum: DrumLayer, core_radius: float, distance_between_drums: float
) -> List[float]:
    drum_radiuses = [outer_drum.distance_from_core]
    while (
        drum_radiuses[-1] - (outer_drum.radius - core_radius) > distance_between_drums
    ):
        drum_radiuses.append(drum_radiuses[-1] - distance_between_drums)
    return drum_radiuses
