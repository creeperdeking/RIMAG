...
def downsample_niel_desc(fname_in,
                         fname_out,
                         npts=30):
    """
    ...
    hdr = (f"E_MeV   NIEL(MeV cm^2 g^-1) – down-sampled to {npts} points "
           f"from '{pathlib.Path(fname_in).name}'")
    np.savetxt(fname_out,
               np.column_stack([E_comp, D_comp]),
               fmt="%.6e %.6e",
               header=hdr)

    print(f"Wrote {npts}-point table to {fname_out} "
          f"({E_min:.2e} – {E_max:.2e} MeV, ascending)")

# USAGE EXAMPLE ---------------------------------------------------------------
downsample_niel_desc("srniel_Si_E722-19.txt",
                     "srniel_Si_E722-19_compact.txt",
                     npts=30)
